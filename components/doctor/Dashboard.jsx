...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch('/api/patients')
      .then((r) => r.json())
      .then((data) => {
        if (!cancelled && Array.isArray(data)) setPatientList(data);
      })
  ...
  const totalPatients = patientList?.length || 0;
  ...
  const todayEarnings = useMemo(() => {
    ...
    return (patientList || []).reduce((sum, patient) => {
      let total = 0;
      if (patient.date === todayKey) total += Number(patient.fee) || 0;
  ...
  const topDiseases = useMemo(() => {
    const counts = (patientList || []).reduce((acc, patient) => {
  ...
  const earningsByDate = useMemo(() => {
    ...
    (patientList ?? []).forEach((patient) => {
      addAmount(patient.date, Number(patient.fee) || 0);
      if (Array.isArray(patient.dateWiseData)) {
  ...
  }, [patientList, chartStartDate, chartEndDate]);
  ...
  const topDiseasesSeries = useMemo(
    () => topDiseases.map((item) => ({ label: item.disease, value: item.count })),
    [topDiseases]
  );

  const activeChartData = useMemo(() => {
    if (metricFilter === 'visits')
      return patientTrendByDate.map((item) => ({ label: item.date, value: item.count }));
    if (metricFilter === 'patients')
      return patientRegistrationsByDate.map((item) => ({ label: item.date, value: item.count }));
  ...
  const chartPatientCount = useMemo(() => {
    ...
    (patientList ?? []).forEach((patient) => {
      ...
      if (
        !Number.isNaN(registerTime) &&
        (!start || registerTime >= start) &&
        (!end || registerTime <= end)
      ) {
        ids.add(patient.id);
      }
      if (Array.isArray(patient.dateWiseData)) {
    ...
    const points = activeChartData.map((item, index) => {
      const x =
        ...
        (activeChartData.length === 1
          ? chartWidth / 2
          : (chartWidth / (activeChartData.length - 1)) * index);
  ...
  return (
    <div>
            ...
            <div className="form-horizon-btw">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Use the date range and chart type to explore clinic data.
                </p>
              </div>
              <div className="text-right text-sm text-slate-500">
                {metricFilter === 'diseases'
                  ? `Total ${activeChartTotal} cases`
                  : `Total ${activeChartTotal.toLocaleString()}`}
              </div>
            </div>
          ...
          <section className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
            ...
            <div className="mt-5 space-y-3 text-sm text-slate-600">
              ...
              <div className="flex items-center justify-between rounded-3xl bg-white px-4 py-3">
                ...
                <span className="font-semibold text-slate-900">₹ {totalEarnings.toLocaleString()}</span>
              </div>
            </div>
          </section>
        </div>
      ...
      <div className="mt-8 grid gap-4 xl:grid-cols-2 px-5 pb-8">
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="form-horizon-btw">
            <div>
              <h2 className="text-lg font-semibold text-slate-900">Patient trend</h2>
              <p className="mt-1 text-sm text-slate-500">
                Daily visits and consultations in the selected range.
              </p>
            </div>
            <div className="text-right text-sm text-slate-500">
          ...
          {patientTrendByDate.length > 0 ? (
            ...
            <div className="mt-6 rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500">
              {loading ? 'Loading…' : 'No trend data available for the selected range.'}
            </div>
          )}
        </section>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="form-horizon-btw">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Most frequent conditions from your clinic records.
              </p>
            </div>
            <div className="text-right text-sm text-slate-500">
          ...
          {topDiseases.length > 0 ? (
            ...
            <div className="mt-5 rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500">
              {loading ? 'Loading…' : 'No disease summary available yet.'}
            </div>
          )}
        </section>
      </div>
