...
export default function LoginForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await signIn('doctor', { username: username.trim(), password, redirect: false });
      if (!res || !res.ok) {
  ...
  const handleSendResetOtp = async () => {
    ...
    try {
      const r = await fetch('/api/auth/otp/send', {
        ...
        body: JSON.stringify({ purpose: 'password_reset', username: resetUsername.trim() }),
      });
      const data = await r.json();
      if (!r.ok) {
        setResetMessage(data.error || 'Failed to send OTP.');
      } else {
        setResetStep(2);
        setResetMessage('OTP sent. Check your email and enter it below.');
      }
    } catch {
      setResetMessage('Unable to send OTP. Please check email settings or contact support.');
    } finally {
  ...
  const handleResetPassword = async () => {
    ...
    setResetLoading(true);
    try {
      const r = await fetch('/api/auth/otp/reset', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <button
                        type="button"
                        onClick={() => setShowResetPassword((c) => !c)}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <button
                        type="button"
                        onClick={() => setShowResetConfirm((c) => !c)}
                        className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs text-slate-700 hover:bg-slate-50"
                      >
